...
void RR(Process *processSet, int totalServiceTime, bool **workload, int timeSlice){
	...
	for(int time=0;time<totalServiceTime;time++){
		...
		if(run == timeSlice){ // time slice만큼 run했는지 확인
			run = 0;
			enQueueRear(&ReadyQ, RunProc); // 뒤로 삽입
		}
		if(run == 0){ // 수행하기 위해 삭제
			RunProc = deQueue(&ReadyQ);
		}
		(RunProc->currentServiceTime)++;
		workload[RunProc->processId][time] = true;//출력을 위한 체크
		run++;
		if(RunProc->currentServiceTime == RunProc->serviceTime){ //serviceTime만큼 수행완료시 
			run = 0;
		}
	}
}
...
void Lottery(Process *processSet, int totalServiceTime, bool **workload){
	...
	for(int time=0;time<totalServiceTime;time++){
		...
		if(!isEmpty(&ReadyQ)){
			lotteryNum = rand()%totalTicket; //난수생성
			...
			for(int i = 0; i<ReadyQ.count;i++){ //어느 노드인지 탐색
				...
				if(search > lotteryNum){ //찾았을 경우
					...
					workload[forSearch->processId][time]=true; // 출력체크
					if(forSearch->currentServiceTime == forSearch->serviceTime){//수행완료시
						totalTicket -= forSearch->ticket;
						deQueueInLottery(&ReadyQ, pa, ch); //삭제
					}
					break;
				}
				pa = ch;
				ch = ch->next;
				forSearch = ch->process; //노드 next
			}
		}
	}
}
